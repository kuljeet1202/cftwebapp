            ...
            <div class="row mb-2">
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active"><i class="nav-icon fas fa-cubes mr-1"></i>{{ __('subcategory') }}</li>
                    </ol>
                </div>
            </div>
        </div>
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12 d-flex justify-content-end">
                    <button id="toggleButton" class="btn btn-primary mb-3 ml-1"><i class="fas fa-plus-circle mr-2"></i>{{ __('create') . ' ' . __('subcategory') }}</button>
                </div>
                <div class="col-md-12" id="add_card">
                    <div class="card card-secondary">
                        ...
                        <form id="create_form" action="{{ route('sub_category.store') }}" role="form" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="card-body">
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-3 col-sm-12">
                                        ...
                                        <input type="text" name="slug" required placeholder="{{ __('slug') }}" class="form-control">
            ...
            <div class="row">
                <div class="col-lg-8 col-md-12 col-sm-12">
                    <div class="card card-secondary">
                        ...
                        <div class="card-body">
                            ...
                            <table aria-describedby="mydesc" id='table' data-toggle="table" data-url="{{ route('subcategoryList') }}" data-click-to-select="true" data-side-pagination="server" data-pagination="true" data-page-list="[5, 10, 20, 50, 100, 200]" data-search="true"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th scope="col" data-field="category_id" data-visible="false" data-sortable="true">{{ __('category_id') }}</th>
                ...
                <div class="col-lg-4 col-md-12 col-sm-12">
                    <div class="card card-secondary">
                        ...
                        <div class="card-body">
                            <form id="order_form" action="{{ route('update_subcategory_order') }}" method="post" onsubmit="return saveOrder()">
                                @csrf
        ...
        <div class="modal fade" id="editDataModal">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form id="update_form" action="{{ url('sub_category') }}" role="form" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="modal-body">
                            <div class="row">
                                <div class="form-group col-md-12 col-sm-12">
                                    <label class="required">{{ __('language') }}</label>
                                    <select id="edit_language_id" name="edit_language" class="form-control" required>
                                        <option value="">{{ __('select') . ' ' . __('language') }}</option>
                                        @foreach ($languageList as $item)
                                            <option value="{{ $item->id }}">{{ $item->language }}</option>
                                        @endforeach
                            ...
                            <div class="row">
                                <div class="form-group col-md-12 col-sm-12">
                                    <label class="required">{{ __('name') }}</label>
                                    <input id="edit_name" name="name" type="text" required class="form-control" placeholder="{{ __('name') }}">
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group col-md-12 col-sm-12">
                                    <label class="required">{{ __('slug') }}</label>
                                    <input type="text" name="slug" id="slug" required class="form-control" placeholder="{{ __('slug') }}">
                                    <span class="text-danger">{{ __('avoid_special_characters') }}</span>
                                </div>
                            </div>
                        </div>
...
@section('script')
    <script type="text/javascript">
        ...
        $(document).on('change', '#edit_language_id', function(e, row_language, row_category) {
            ...
            var url = '{{ route('get_category_by_language') }}';
            fetchList(url, data, '#edit_category_id');
            setTimeout(() => {
