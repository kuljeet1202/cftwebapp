...
class TagController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $rows = $sql->get()->map(function ($row) {
            ...
            return [
                ...
                'image' => !empty($row->og_image) ? '<a href="' . $row->og_image . '" data-toggle="lightbox" data-title="Image"><img class="images_border" src="' . $row->og_image . '" height="50" width="50"></a>' : '-',
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('file')) {
            $tag->og_image = $request->file('file')->store('tag_og_image', 'public');
        } else {
            $tag->og_image = '';
        }
        ...
        $tag->meta_keyword = get_meta_keyword($meta_keyword);
        $tag->save();
        $response = [
            'error' => false,
